...
namespace EasyBuildSystem.Packages.Addons.AdvancedBuilding
{
    public class CarriableController : MonoBehaviour
    {
        ...
        [SerializeField] UnityEngine.InputSystem.Key m_DropKey = UnityEngine.InputSystem.Key.G;
#else
        ...
        {
#if EBS_INPUT_SYSTEM_SUPPORT
            if (UnityEngine.InputSystem.Keyboard.current[m_DropKey].wasPressedThisFrame && CarriableCount > 0)
            {
                Drop(m_DropForce);
            }
#else
